/*eslint-disable*/
import { login, logout } from "@/Apis/User/authApis";
const AuthStore = {
  ...
  actions: {
        ...
        login(queries)
          .then((resp) => {
            if (resp) {
              ...
              localStorage.setItem("loginDetails", JSON.stringify(x.user));
    ...
    LOGOUT({ commit }) {
      logout()
        .then((resp) => {
          localStorage.removeItem("loginDetail"); //if you wish to user local storage for persistent login uncomment
